...
from service.service.UserService import UserService,get_table
...
class UserListCtl(BaseCtl):
    ...
    def previous(self, request, params={}): 
        ...
        record,last_pg_no = get_table(db_model_class=self.get_service().get_model(),
                                      ...
                                      pzsz = self.pg_size,
                                      filter_param=srch_prm)
        # record,last_pg_no = get_table(self.get_service().get_model(),'firstName',prevPageNo,self.pg_size)
        res = render(request, self.get_template(), {'pageList':record, "form": self.form})
        return res
    ...
    def deleteRecord(self, request, params={}):
        ...
        if (bool(self.form['ids']) == False):
            ...
            record,last_pg_no = get_table(db_model_class=self.get_service().get_model(),
                                      ...
                                      pzsz = self.pg_size,
                                      filter_param=srch_prm)
            # record,last_pg_no = get_table(self.get_service().get_model(),'firstName',pageNo,self.pg_size)
            res = render(request, self.get_template(), {'pageList':record, "form": self.form})
            return res
        else:
